...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      this.axios.get(`http://127.0.0.1:666/account/accountedit?id=${currentid}`)
      .then(response=>{
    ...
    submitForm(formName){
      this.$refs[formName].validate(valid =>{
        if(valid){
          ...
          let parms = {id:currentid,username:username,usergroup:usergroup}
          //post 请求发给后台
          this.axios.post('http://127.0.0.1:666/account/accounteditconfirm',qs.stringify(parms))
          .then(response=>{
